...
```mermaid
flowchart LR
    U[메인 챗에<br/>요구사항 입력] --> S[이슈를<br/>서브 이슈 2개로 분해]
    S --> W1[서브 이슈 1<br/>워크트리]
    S --> W2[서브 이슈 2<br/>워크트리]
    W1 --> C1[Claude 실행]
    W2 --> C2[Claude 실행]
    C1 --> D[대시보드<br/>이슈 · 시간 · 토큰 · 진행률]
    C2 --> D

    classDef user fill:#eef,stroke:#557,color:#000
    classDef agent fill:#efe,stroke:#393,color:#000
    classDef dash fill:#ffd,stroke:#a80,color:#000
    class U user
    class C1,C2 agent
    class D dash
```
